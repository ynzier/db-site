...
const App = props => {
  ...
  const preData = async () => {
    ...
    await transData.forEach(async item => {
      var data = {
        prod_id: item[2],
        quantity: item[3],
      };
      await ProductDataService.updateStock(data)
        .then(response => {
    ...
    await CustomerDataService.createTransaction(transData, total)
      .then(response => {
        refreshList();
      })
      .catch(error => {
  ...
  const refreshList = async () => {
    await CartService.getCart(JSON.parse(localStorage.getItem('user')).custNo)
      .then(res => {
        setRecord(res.data);
        var sum = res.data.reduce((a, b) => a + b.quantity * b.price, 0);
        setTotal(sum);
